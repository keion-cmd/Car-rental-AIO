...
export async function checkOutBookingAction(formData: FormData): Promise<void> {
  ...
  const outcome = await checkOutBooking(bookingId, { odometerOut, fuelOut, staffUserId: user.id });
  if (!outcome.ok) {
...
export async function checkInBookingAction(formData: FormData): Promise<void> {
  ...
  const outcome = await checkInBooking(bookingId, { odometerIn, fuelIn, staffUserId: user.id });
  if (!outcome.ok) {
